...
class Decoder_Index():
    def __init__(self, cfg) -> None:
        super(Decoder_Index, self).__init__()
    ...
    def inference_index(self, one_grad, token_indices, num_inference_steps=50, guidance_scale = 7.5):
        ...
        embedding = self.text_encoder(token_indices.unsqueeze(0))[0].to(dtype=self.unet.dtype, device=self.device)
    ...
    def decode(self, images, one_grad, token_indices):
        ...
        # get embedding
        prompt_embeds = self.text_encoder(token_indices.unsqueeze(0))[0].to(dtype=self.unet.dtype, device=self.device)
