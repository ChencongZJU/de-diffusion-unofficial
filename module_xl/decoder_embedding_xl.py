...
class Decoder_Embedding_XL():
    def __init__(self, cfg) -> None:
        super(Decoder_Embedding_XL, self).__init__()
    ...
    def inference_embedding(self, prompt_embeds, num_inference_steps=30, guidance_scale = 7.5):
        # Prepare latents
        ...
        pooled_prompt_embeds = prompt_embeds[:, -1, 768:]
    ...
    def decode(self, images, embedding):
        ...
        pooled_prompt_embeds = embedding[:, -1, 768:]
        add_text_embeds = pooled_prompt_embeds
        add_time_ids = torch.tensor(list(self.original_size + self.crops_coords_top_left + self.target_size)).to(self.device)
        ...
        model_pred = self.unet(noisy_latents, timesteps, embedding, added_cond_kwargs=added_cond_kwargs).sample
