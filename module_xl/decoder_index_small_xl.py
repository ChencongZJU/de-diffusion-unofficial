...
class Decoder_Index_Small_XL():
    def __init__(self, cfg) -> None:
        super(Decoder_Index_Small_XL, self).__init__()
    ...
    def generate_small_vocabulary(self, prompt):
        ...
        embeddings1 = copy.deepcopy(self.text_encoder.text_model.embeddings)
        embeddings2 = copy.deepcopy(self.text_encoder_2.text_model.embeddings)
        
        mytokenembedding1 = torch.nn.Embedding(self.small_token_lens, self.cfg.feature_dim)
        mytokenembedding1.weight = nn.Parameter(self.text_encoder.text_model.embeddings.token_embedding(whole_index[0].to(self.device)))
        embeddings1.token_embedding = mytokenembedding1
        
        mytokenembedding2 = torch.nn.Embedding(self.small_token_lens, self.cfg.feature_dim)
        mytokenembedding2.weight = nn.Parameter(self.text_encoder_2.text_model.embeddings.token_embedding(whole_index[0].to(self.device)))
        embeddings2.token_embedding = mytokenembedding2
        
        self.text_encoder.text_model.embeddings = embeddings1
        self.text_encoder_2.text_model.embeddings = embeddings2
